...
def is_torchvision_v2_available():
    ...
    major_version = int(torchvision.__version__.split('.')[1])
...
class LlavaOnevisionVideoProcessorCPU(BaseImageProcessor):
...
class LlavaOnevisionVideoProcessorGPU(BaseImageProcessor):
    ...
    def _preprocess(
        ...
        if image_type == ImageType.NUMPY:
            images = [
                torch.from_numpy(image).contiguous().to(device="cuda")
                for image in images
            ]
...
AutoImageProcessor.register("LlavaOnevisionVideoProcessorCPU",
                            LlavaOnevisionVideoProcessorCPU)
AutoImageProcessor.register("LlavaOnevisionVideoProcessorGPU",
                            LlavaOnevisionVideoProcessorGPU)