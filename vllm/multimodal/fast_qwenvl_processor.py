...
def make_batched_images(images) -> List[List[ImageInput]]:
    ...
    """
    if isinstance(images, (list, tuple)) and isinstance(
            images[0], (list, tuple)) and is_valid_image(images[0][0]):
...
class Qwen2VLImageProcessorFastCPU(BaseImageProcessor):
    ...
    def _preprocess(
        ...
        if patches.shape[0] == 1:
            patches = patches.repeat_interleave(self.temporal_patch_size,
                                                dim=0)
...
class Qwen2VLImageProcessorGPU(BaseImageProcessor):
    ...
    def _preprocess(
        ...
        if input_data_format == ChannelDimension.LAST:
            images = [
                image.permute(2, 0, 1).contiguous().to(device="cuda")
                for image in images
            ]
            input_data_format = ChannelDimension.FIRST
